...
import { X, Download, ShieldAlert, Activity, RefreshCw, Search } from "lucide-react";
...
export function EntityDetailsOverlay({ entityId, onClose, onInvestigate }: EntityDetailsOverlayProps) {
  ...
  data?.transactions?.forEach((t: any) => {
    if (t.type === "C" || t.type === "Credit") typeCount.C += 1;
    else typeCount.D += 1;
    if (t.counterparty) {
  ...
  const auditReportHtml = (data?.audit_report || "• **Forensic Baseline**: Analysis completed. Direct records cataloged in master intelligence registry.")
    .replace(/\n/g, "<br/>")
      ...
      <div className="w-full max-w-5xl h-[90vh] bg-[#0a0e1a] rounded-xl border border-slate-700/80 shadow-2xl flex flex-col overflow-hidden relative">
        ...
        <div className="flex items-center justify-between p-4 border-b border-slate-700/80 bg-slate-900/50">
          ...
          <div className="flex items-center gap-3">
            {onInvestigate && (
              <Button 
                onClick={() => onInvestigate(entityId)}
                variant="outline" 
                size="sm"
                className="border-cyan-500/40 text-cyan-400 hover:bg-cyan-950/20 font-mono text-xs"
              >
                <Search className="w-3.5 h-3.5 mr-1.5" /> Investigate in Copilot
              </Button>
            )}
            <Button
              onClick={generatePDF}
              ...
              <div className="col-span-2 space-y-4">
                <div className="p-5 rounded-lg border border-cyan-900/50 bg-cyan-950/20 h-full">
                  ...
                  <div className="prose prose-invert prose-sm max-w-none font-sans text-slate-300 leading-relaxed" 
                       dangerouslySetInnerHTML={{ __html: auditReportHtml }} />
                </div>
              </div>
              ...
              {((data?.transactions?.length || 0) > 0) && (
                <div>
                  ...
                  <div className="overflow-x-auto">
                    ...
                    {(data?.transactions?.length || 0) > 15 && <p className="text-xs text-slate-500 mt-2 italic">+ {(data?.transactions?.length || 0) - 15} more records omitted.</p>}
                  </div>
                </div>
              )}
                    ...
                    <table className="w-full text-xs text-left">
                      ...
                      <tbody>
                        {data?.ips?.slice(0, 10).map((p: any, i: number) => (
                          <tr key={i} className="border-b border-slate-800/50 hover:bg-slate-800/20">
                            <td className="py-2 px-3 font-mono">{p.date} {p.time}</td>
                            <td className="py-2 px-3 font-mono text-cyan-400">{p.destination}</td>
