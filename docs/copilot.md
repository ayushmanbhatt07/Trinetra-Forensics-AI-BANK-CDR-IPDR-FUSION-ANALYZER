...
5. **LLM Generation**: The prompt, containing the user's question and the strict JSON evidence, is sent to the LLM (powered by OpenRouter / NVIDIA Nemotron).
...
## 3. OpenRouter API Key Management
The Copilot utilizes the OpenRouter inference engine for high-speed LLM generation.

* **Storage**: Keys are stored in `backend/.env` using the format `OPEN_ROUTER_KEY_{N}`.
* **Rotation**: The backend automatically loads all keys and rotates through them sequentially to load-balance requests.
* **Fallback**: If a key hits a rate limit, the system automatically falls back to the next available key or fallback model. If all keys fail, the UI elegantly notifies the user or falls back to basic deterministic search.
